...
if (empty($fileId) || !isset($files[$fileId])) {
    ...
    if (!file_exists($filePath)) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password'])) {
            if (password_verify($_POST['password'], $file['password'])) { $needsPassword = false; $downloadReady = true; }
            else { $error = 'Yanlış şifre!'; }
        }
    } else { $downloadReady = true; }
...
<body>
    <div class="container">
        <header><a href="/" class="logo-link"><h1><?= SITE_NAME ?></h1></a></header>
        ...
        <div class="download-section">
            <div class="file-preview">
                ...
                <div class="file-stats"><span><?= formatFileSize($file['size']) ?></span><span>•</span><span><?= date('d.m.Y H:i', strtotime($file['upload_date'])) ?></span><span>•</span><span><?= $file['downloads'] ?> indirme</span></div>
            </div>
            <?php if ($needsPassword): ?>
            <div class="password-form">
                <?php if ($error): ?><div class="message error"><?= $error ?></div><?php endif; ?>
                <p>Bu dosya şifre ile korunuyor.</p>
                <form method="POST"><input type="password" name="password" placeholder="Şifreyi girin" required autofocus><button type="submit" class="btn-upload">Şifreyi Doğrula</button></form>
            </div>
            <?php else: ?>
            <a href="?id=<?= $fileId ?>&download=1" class="btn-download"><span class="download-icon">⬇️</span> İndir</a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
